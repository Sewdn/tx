...
export function LexiconSidebar({
  ...
  return (
    <aside
      className={cn(
        "fixed top-0 left-0 z-[60] flex h-screen w-64 flex-col border-r border-outline-variant bg-surface py-region dark:border-outline dark:bg-surface-dim",
        className,
      )}
    >
      ...
      <nav className="grow">
        <ul className="space-y-micro">
          {navItems.map((item) => (
            <li key={item.id} className="px-component">
              <a
                href={item.href}
                className={cn(
                  "flex items-center gap-component rounded p-micro font-ui-label-md transition-all duration-200",
                  item.active
      ...
      <div className="mt-auto px-section">
        <Button className="flex w-full items-center justify-center gap-micro bg-primary py-component font-ui-label-md text-on-primary hover:opacity-90">
          {primaryActionLabel}
        </Button>
        {footer}
      </div>

...
export function BuildCatalogCard({
  ...
  return (
    <article
      className={cn(
        "group relative flex flex-col gap-6 border border-outline-variant bg-surface-container-low p-6 transition-all duration-500 hover:border-outline hover:bg-surface-container md:flex-row",
        "book-card-shadow",
        className,
      )}
    >
      <div className="relative h-60 w-40 shrink-0 overflow-hidden transition-transform duration-500 group-hover:-translate-y-2">
        ...
        {badge ? (
          <div className="absolute top-2 right-2 rounded-sm bg-secondary px-2 py-0.5 font-ui-label-sm text-ui-label-sm text-on-secondary">
            {badge}
          </div>
        ) : null}
      </div>
      <div className="flex flex-1 flex-col justify-between">
        <div>
          ...
          <p className="mb-4 font-body-md text-body-md text-on-surface-variant italic">{author}</p>
          <div className="mb-6 flex items-center gap-2">
            <span className="rounded border border-outline-variant bg-surface-container-highest px-2 py-1 font-code-sm text-code-sm text-primary">
              {commit}
            </span>
        ...
        <div className="flex gap-3">
          <Button className="flex-1 bg-primary py-3 font-ui-label-md text-on-primary hover:bg-primary-container">
            Read Now
          </Button>

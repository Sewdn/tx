...
- **`domain/`** (repo root) — Gittenberg GraphQL SDL (`domain/graphql/`). Validate with **`tx-admin domain validate-schema`** (`@tx/cli-admin`). Runtime entities live in **`packages/domain-shared`**.
- **`packages/`** — Shared code: **`domain-shared`** (Effect entities/ports), **`svc-config`** and **`svc-*`** (backend services; keep data access behind a service layer from API apps), **`ui`** (Shadcn primitives only), **`ui-lib`** and **`ui-*`** (composed UI; depend on **`ui`** / **`ui-lib`**, never on **`svc-*`**), **`typescript-config`**, and other packages added via the CLI. **Never** add a **`svc-*`** dependency to a **`ui-*`** package, or a **`ui-*`** dependency to a **`svc-*`** package.
- **`apps/cli-admin`** — Operational/admin CLI (`tx-admin`); consolidate schema checks and other admin tasks here instead of one-off scripts in source trees.
...
- **For API routes**, use scaffold expansion commands (e.g. `scaffold app-module add-crud-module <entity> --service <svc> --app <api-app>`) before customizing.
...
For this repository itself, keep the local Dora index fresh: scaffold commands trigger a best-effort Dora reindex after each run, and scaffolded projects install a Git **pre-commit** hook that runs `node .scripts/dora-reindex.mjs` before commits.
